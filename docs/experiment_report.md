...
| :--- | :--- | :---: | :---: | :---: | :--- |
| 🥉 **AudioResNet-50** | 2D CNN (ImageNet 사전학습) | 23.50M | **91.20%** | **0.9118** | 🏢 **풀학습 완료 (90.20% ➔ 91.20% +1.00%p 상승!)** |
| 🥈 **ReDimNet2-B2** | Hybrid (2D+1D+Attention) | **2.57M** | **91.89%** | **0.9185** | 🚀 **초경량 풀학습 (9.8MB, 2.05ms) 2위 달성** |
...
| 🥉 **AudioResNet-50** | 2D CNN (ImageNet 전이) | **23.50M** | Local RTX 3060 | 10 ep (풀학습) | **약 22.0시간** | **약 132분** | ~4.8 GB | **91.20%** | 🏢 **[비전 앵커]** 90.20%에서 풀학습으로 +1.00%p 향상 |
...
| 🥉 **AudioResNet-50** | **23.50M** | **90.0 MB** | **5.11 ms** | **0.0017** | **640.9 samples/s** | **91.20%** | 🏢 **안정적인 고사양 서버 앙상블용** |

> 💡 **RTF (Real-Time Factor)의 의미**:  
> $\text{RTF} = \frac{\text{추론 처리 시간}}{\text{실제 오디오 길이(3초)}}$.  
> RTF가 1.0보다 작아야 실시간 스트리밍 음성을 끊김 없이 판별할 수 있습니다. **3개 모델 모두 RTF < 0.002로 실제 사람 말소리보다 500~1,400배 빠르게 실시간 분류**합니다!

---

### (2.9) 🔥 3대장 (ECAPA 92.10% vs ReDimNet 91.89% vs ResNet 91.20%) 정밀 맞대결 분석

| 비교 항목 | 🥇 ECAPA-TDNN | 🥈 ReDimNet2-B2 | 🥉 AudioResNet-50 | 종합 분석 및 앙상블 기여도 |
| :--- | :---: | :---: | :---: | :--- |
| **순수 정확도 (Val Acc)** | **92.10%** (Ep 08) | **91.89%** (Ep 09) | **91.20%** (Ep 10) | 🏆 **3대 모델 모두 91.2% 돌파!** ECAPA가 화자 성대 분산 요약으로 1위 |
| **단일 추론 지연시간** | 5.42 ms | **2.05 ms** | 5.11 ms | ⚡ **ReDimNet 2.6배 빠름**: 2ms 초고속 기동성 |
| **가중치 파일 용량** | 22.2 MB (5.80M) | **9.8 MB (2.57M)** | 90.0 MB (23.50M) | 📦 **ReDimNet이 ResNet 대비 9배 가벼움** |
| **초당 서버 처리량** | 1,334.5 건/초 | **2,047.2 건/초** | 640.9 건/초 | 🚀 **ReDimNet > ECAPA > ResNet 순서** |
| **아키텍처 관점** | 1D 시간 지연 합성곱 + Stats Pool | 2D Conv + Multi-Head Attn | 2D Deep CNN (7x7 Conv + ResBlock) | 🤝 **3종류의 서로 완전히 다른 메커니즘 ➔ 앙상블 시너지 극대화** |
