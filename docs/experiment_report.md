...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| 🥇 **ReDimNet2-B2** | Hybrid (2D+1D+Attn) | **2.57M** | Local RTX 3060 | 10 ep (풀학습) | **약 17.5시간** | **약 105분** | ~3.5 GB | **91.89%** | **가성비 압도적 1위** (F1 91.89% 신기록 경신) |
| 🥈 **ECAPA-TDNN** | 1D CNN + Stats Pool | **5.80M** | Local RTX 3060 | 5 ep (벤치마크) | **40.95분** | **약 8.2분** | ~3.5 GB | **0.8691** | **가성비 2위** (화자 임베딩에 최적화, 풀학습 진행 중) |
| 🥉 **AudioResNet-50** | 2D CNN (ImageNet 전이) | **23.50M** | Local RTX 3060 | 5 ep | **약 85분** | **약 17.0분** | ~4.8 GB | **0.9018** | **안정적 성능 보증** (90%선 든든한 1차 앵커 모델) |
...
| 모델명 | 파라미터 수 | 체크포인트 크기 | 단일 추론 지연시간 (Latency) | 실시간 지수 (RTF) | 초당 처리량 (Throughput) | 119 현장 배포 적합도 |
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| 🥇 **ReDimNet2-B2** | **2.57M** | **9.8 MB** | **2.05 ms** | **0.0007** | **2,047.2 samples/s** | 🚀 **초실시간 온디바이스(무전기/구급차) 최적** |
| 🥈 **ECAPA-TDNN** | **5.80M** | **22.2 MB** | **5.42 ms** | **0.0018** | **1,334.5 samples/s** | ✨ **초고속 상황실 실시간 다채널 모니터링** |
| 🥉 **AudioResNet-50** | **23.50M** | **90.0 MB** | **5.11 ms** | **0.0017** | **640.9 samples/s** | 🏢 **안정적인 고사양 서버 추론용** |

> 💡 **RTF (Real-Time Factor)의 의미**:  
> $\text{RTF} = \frac{\text{추론 처리 시간}}{\text{실제 오디오 길이(3초)}}$.  
> RTF가 1.0보다 작아야 실시간 스트리밍 음성을 끊김 없이 판별할 수 있습니다. **ReDimNet의 RTF 0.0007은 실제 사람 말소리 재생 속도보다 무려 1,400배 이상 빠르게 판단**한다는 의미입니다!

---

### (2.8) 📐 다차원 효율성 평가 기준 5대 프레임워크 (5-Dimension Framework)
실전 배포와 대회 평가에서 종합 가성비를 따지기 위해 앞으로 기록해야 할 **5대 핵심 평가 축**:
...
1. **정확성 (Accuracy & F1-Score)**: 클래스 불균형에 왜곡되지 않는 검증 데이터셋의 Macro F1.
2. **반응 속도 (Latency & RTF)**: 119 신고 전화 수신 즉시 0.01초 내에 화자를 분별할 수 있는 실시간성.
3. **서버 처리량 (Throughput)**: 1개 GPU로 수천 통의 119 통화를 동시 수용할 수 있는 동시성 능력.
4. **온디바이스 경량성 (Model Footprint)**: 구급차 태블릿이나 무전기 내장 NPU에 10MB 이하로 탑재 가능한가.
5. **학습 비용 및 전력 (Training Cost & Energy)**: 연구/재학습 시 발생하는 시간적 지연 및 전력 비용.
