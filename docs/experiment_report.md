...
| :--- | :--- | :---: | :---: | :---: | :--- |
| **AudioResNet-50** | 2D CNN (ImageNet 사전학습) | 23.50M | **90.20%** | **0.9018** | 든든한 1차 비전 베이스라인 |
| 🥈 **ReDimNet2-B2** | Hybrid (2D+1D+Attention) | **2.57M** | **91.89%** | **0.9185** | 🚀 **초경량 풀학습 1차 신기록 경신** |
| 🥇 **ECAPA-TDNN** | 1D CNN + Stats Pooling | **5.80M** | **92.10%** | **0.9208** | 🏆 **[전체 1위] 단일 모델 역대 최고점 달성!** |
...
| 모델명 | 아키텍처 | 파라미터 수 | 실행 환경 | 학습 에폭 | 총 소요시간 | 에폭당 평균 시간 | 피크 VRAM | 최종 Accuracy | 자원 대비 효율성 (가성비) |
| :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| 🥇 **ECAPA-TDNN** | 1D CNN + Stats Pool | **5.80M** | Local RTX 3060 | 10 ep (풀학습) | **약 20.0시간** | **약 120분** | ~3.5 GB | **92.10%** | 🏆 **[전체 1위] 단일 모델 역대 최고 기록 경신!** |
| 🥈 **ReDimNet2-B2** | Hybrid (2D+1D+Attn) | **2.57M** | Local RTX 3060 | 10 ep (풀학습) | **약 17.5시간** | **약 105분** | ~3.5 GB | **91.89%** | 🥇 **[가성비 1위]** 2.05ms 초고속, 9.8MB 초경량 |
| 🥉 **AudioResNet-50** | 2D CNN (ImageNet 전이) | **23.50M** | Local RTX 3060 | 5 ep | **약 85분** | **약 17.0분** | ~4.8 GB | **90.20%** | **안정적 성능 보증** (90%선 든든한 1차 앵커 모델) |
| **Wav2Vec 2.0** | Raw Waveform Transformer | **95.0M** | Colab (T4) | 3 ep | **약 140분** | **약 46.7분** | ~11.5 GB | **89.72%** | 앙상블(91.05%)용 핵심이나, 단독 훈련 시간 큼 |
| **SSAST** | Audio Spectrogram ViT | **85.0M** | Colab (T4) | 1 ep | **약 65분** | **약 65.0분** | ~10.8 GB | 81.20% | 패치 어텐션 $O(N^2)$ 계산 복잡도로 시간 소모 과다 |
| **HuBERT-Base** | Hidden-Unit Transformer | **95.0M** | Colab (T4) | 1 ep 중단 | **> 90분 (중단)** | > 90.0분 | > 13.0 GB | - (미완료) | ❌ **비효율** (가중치 다운로드 + T4 세션 타임아웃) |

---

### (2.5) ⚡ [실측] 추론 지연시간(Latency), 실시간성(RTF) 및 경량성 벤치마크
> **측정 환경**: NVIDIA GeForce RTX 3060 Laptop GPU (CUDA 11.8)  
> **입력 규격**: 3.0초 오디오 발화 세그먼트 (200회 반복 추론 정밀 측정)

| 모델명 | 파라미터 수 | 체크포인트 크기 | 단일 추론 지연시간 (Latency) | 실시간 지수 (RTF) | 초당 처리량 (Throughput) | 풀학습 최고 정확도 | 119 현장 배포 적합도 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| 🥇 **ReDimNet2-B2** | **2.57M** | **9.8 MB** | **2.05 ms** | **0.0007** | **2,047.2 samples/s** | **91.89%** | 🚀 **초실시간 온디바이스(무전기/구급차) 최적** |
| 🥈 **ECAPA-TDNN** | **5.80M** | **22.2 MB** | **5.42 ms** | **0.0018** | **1,334.5 samples/s** | **92.10%** | 🏆 **정확도 1위 & 초고속 다채널 상황실 관제** |
| 🥉 **AudioResNet-50** | **23.50M** | **90.0 MB** | **5.11 ms** | **0.0017** | **640.9 samples/s** | **90.20%** | 🏢 **안정적인 고사양 서버 앙상블용** |

> 💡 **RTF (Real-Time Factor)의 의미**:  
> $\text{RTF} = \frac{\text{추론 처리 시간}}{\text{실제 오디오 길이(3초)}}$.  
> RTF가 1.0보다 작아야 실시간 스트리밍 음성을 끊김 없이 판별할 수 있습니다. **ReDimNet은 실제 사람 말소리보다 1,400배, ECAPA는 550배 이상 빠르게 판별**합니다!

---

### (2.9) 🔥 ReDimNet(91.89%) vs ECAPA-TDNN(92.10%) 2대장 정밀 맞대결 분석

| 비교 항목 | 👨‍🚒 ECAPA-TDNN (92.10%) | ⚡ ReDimNet2-B2 (91.89%) | 우승 모델 및 분석 요인 |
| :--- | :---: | :---: | :--- |
| **순수 정확도** | **92.10% (Epoch 8)** | 91.89% (Epoch 9) | 🏆 **ECAPA 우세 (+0.21%p)**: 통계적 풀링(Stats Pool)이 3초 전체의 음색 평균·표준편차를 완벽 추출 |
| **단일 추론 속도** | 5.42 ms | **2.05 ms** | 🏆 **ReDimNet 압승 (2.6배 빠름)**: 2ms 만에 즉각 판단하는 극강의 기동성 |
| **모델 가벼움** | 22.2 MB (5.8M) | **9.8 MB (2.57M)** | 🏆 **ReDimNet 압승 (2.3배 작음)**: 10MB 이하로 무전기/구급차 임베디드 장치에 적합 |
| **초당 서버 처리량** | 1,334.5 건/초 | **2,047.2 건/초** | 🏆 **ReDimNet 압승 (+53% 더 많은 통화 수용)** |
| **상호 시너지** | 1D 시간 지연 합성곱 + SE-Net | 2D 주파수 Conv + Multi-Head Attn | 🤝 **환상의 앙상블 조합**: 아키텍처가 완전히 달라 Soft Voting 결합 시 폭발적 시너지 발생! |

---

### (3) 가설 4 (A/B 특화 전처리) 실험 추적 로깅 템플릿

실측된 풀학습 성적과 향후 특화 전처리 실험을 비교 기록하는 표준 트래킹 테이블입니다:

| 실험 ID | 실험 조건 | 모델 | 실행 환경 | 배치 크기 | 에폭 수 | 총 학습 시간 | 피크 VRAM | Val Accuracy | 비고 및 코멘트 |
| :---: | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **Exp 0-A** | **순정 Mel 풀학습 (완료)** | **ReDimNet-B2** | Local (RTX 3060) | 32 | 10 ep | 약 17.5시간 | ~3.5 GB | **91.89%** | 🥇 **[초경량 1위]** 2.05ms / 9.8MB / `best_redimnet.pt` 저장 |
| **Exp 0-B** | **순정 Mel 풀학습 (완료)** | **ECAPA-TDNN** | Local (RTX 3060) | 32 | 10 ep | 약 20.0시간 | ~3.5 GB | **92.10%** | 🏆 **[전체 1위 신기록!]** 단일 모델 최고점 경신 / `best_ecapa_tdnn.pt` 저장 |
| **Exp 0-C** | **2대장 Soft Voting 앙상블** | **ReDim + ECAPA** | Local (RTX 3060) | - | 추론 전용 | ~2분 | ~3.5 GB | **측정 대기** | 🚀 **[다음 즉시 실행 가능]** 91.89% + 92.10% 가중 평균 |
...
| **Ensemble** | ResNet + ReDim + ECAPA | 3종 결합 | Local (RTX 3060) | - | 추론 전용 | ~3분 | ~4.5 GB | 목표 **93~95%+** | 3대장 Soft Voting + Threshold Tuning |
