...
class HuBERTClassifier(nn.Module):
    """
    HuBERT (Hidden-Unit BERT) 기반 음성 화자 분류기.
    입력: Raw Waveform 1D 텐서 (B, samples=48000)
    대규모 사전학습 트랜스포머 가중치 활용 + Mean Pooling + Linear 분류 헤드
    """
    def __init__(self, model_name_or_path="facebook/hubert-base-ls960", num_classes=2, freeze_feature_extractor=True):
        super().__init__()
        try:
